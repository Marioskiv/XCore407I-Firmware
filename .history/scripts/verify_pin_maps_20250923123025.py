...
FILE_FULL      = ROOT / 'scripts' / 'pin_map_full.csv'
SCRIPT_FULL    = ROOT / 'scripts' / 'extract_full_pin_map.py'
...
def main():
    ...
    run([sys.executable, str(SCRIPT_FULL), '--out', str(FILE_FULL)])
    ...
    ok &= compare(FILE_FULL, FILE_FULL.read_text(encoding='utf-8'))
    if not ok:

...
void HAL_PCD_MspInit(PCD_HandleTypeDef* pcdHandle)
{
    ...
    /* Bias OTG pins for stable role detection on this board:
     *  - PA10 (ID) pull-up -> prefer B-device (peripheral) semantics if sampled.
     *  - PA9 (VBUS) pulldown -> avoid floating when cable absent.
     * Note: with vbus_sensing disabled this is just to keep them in known states. */
...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev)
{
    ...
    HAL_PCD_DevDisconnect(&hpcd_USB_OTG_FS);
    HAL_Delay(120); /* give host time to register detach */
    return USBD_OK;
}
...
void HAL_PCD_SOFCallback(PCD_HandleTypeDef *hpcd){ s_usbd_sof_seen = 1; USBD_SOF(hpcd->pData); }

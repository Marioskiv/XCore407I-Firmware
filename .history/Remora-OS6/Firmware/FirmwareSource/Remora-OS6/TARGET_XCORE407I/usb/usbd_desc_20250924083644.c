...
{ *length = sizeof(USBD_DeviceDesc); return USBD_DeviceDesc; }

uint8_t *USBD_FS_LangIDStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length)
{ static uint8_t str[] = {0x04, USB_DESC_TYPE_STRING, LOBYTE(USBD_LANGID_STRING), HIBYTE(USBD_LANGID_STRING)}; *length=sizeof(str); return str; }
...
{ return USBD_GetString((uint8_t*)USBD_PRODUCT_STRING, USBD_StrDesc, length); }
uint8_t *USBD_FS_ManufacturerStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length)
{ return USBD_GetString((uint8_t*)USBD_MANUFACTURER_STRING, USBD_StrDesc, length); }
uint8_t *USBD_FS_ConfigStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length)
{ return USBD_GetString((uint8_t*)USBD_CONFIGURATION_STRING, USBD_StrDesc, length); }
uint8_t *USBD_FS_InterfaceStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length)
{ return USBD_GetString((uint8_t*)USBD_INTERFACE_STRING, USBD_StrDesc, length); }

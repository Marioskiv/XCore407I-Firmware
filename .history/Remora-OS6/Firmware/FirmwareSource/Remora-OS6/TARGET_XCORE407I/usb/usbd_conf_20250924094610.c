...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev)
{
    ...
    hpcd_USB_OTG_FS.Init.vbus_sensing_enable = DISABLE; /* no VBUS sense on core board */
    ...
    /* Explicitly assert connection state when VBUS sense is disabled */
    HAL_PCDEx_SetConnectionState(&hpcd_USB_OTG_FS, 1);

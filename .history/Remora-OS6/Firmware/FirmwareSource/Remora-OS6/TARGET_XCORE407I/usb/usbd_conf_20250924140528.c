...
/* Track whether SOF frames have been seen (host active) */
static volatile uint8_t s_usbd_sof_seen = 0;
/* Minimal debug toggle */
#ifndef USB_DEBUG_UART
#define USB_DEBUG_UART 1
#endif

/* Helper APIs used by application */
uint8_t USBD_FS_HasSOF(void) { return s_usbd_sof_seen; }
void USBD_FS_ForceReconnect(void) {
    /* Toggle DP pull-up to force host re-enumeration */
    HAL_PCD_DevDisconnect(&hpcd_USB_OTG_FS);
    HAL_Delay(250);
    HAL_PCD_DevConnect(&hpcd_USB_OTG_FS);
#if USB_DEBUG_UART
    printf("[USB] ForceReconnect pulse\n");
#endif
}
...
void HAL_PCD_MspInit(PCD_HandleTypeDef* pcdHandle)
{
    ...
    /* Per checklist: leave PA9 (VBUS) and PA10 (ID) as inputs, no pull. */
...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev)
{
    ...
    /* Force manual connect regardless of PA9 level (power switch may affect sensing) */
    ...
    HAL_PCD_DevDisconnect(&hpcd_USB_OTG_FS);
    HAL_Delay(250); /* give host time to register detach */
#if USB_DEBUG_UART
    printf("[USB] LL_Init: FS core init, vbus_sense=DIS, FIFOs set, detach asserted\n");
#endif
    return USBD_OK;
}
...
USBD_StatusTypeDef USBD_LL_Start(USBD_HandleTypeDef *pdev)
{
    ...
    /* Ensure device connect (assert DP pull-up) with a slightly longer delay */
    HAL_Delay(80);
    HAL_PCD_DevConnect(&hpcd_USB_OTG_FS);
#if USB_DEBUG_UART
    printf("[USB] LL_Start: PCD started, connect asserted\n");
#endif
    return USBD_OK;
}
...
#if USB_DEBUG_UART
    printf("[USB] DevConnected\n");
#endif
    USBD_LL_DevConnected(hpcd->pData);
}
void HAL_PCD_DisconnectCallback(PCD_HandleTypeDef *hpcd){
#if USB_DEBUG_UART
    printf("[USB] DevDisconnected\n");
#endif

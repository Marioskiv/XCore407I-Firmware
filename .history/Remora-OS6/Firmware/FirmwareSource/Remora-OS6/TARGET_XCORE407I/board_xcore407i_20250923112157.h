 ...
 *  TX_EN    PG11 (default) or PB11 (if built with USE_PG11_TXEN=0)
 *  PHY_RST  PG13 (dedicated reset) NOTE: Some schematic variants tie RESET_N to NRST; if so set ETH_PHY_HAS_DEDICATED_RESET=0
 *  PHY_INT  (INT/PWRDOWN optional; not wired in provided JSON => disabled)
 *  Notes:
 *   - `USE_PG11_TXEN` (default 1) selects PG11; set to 0 to force legacy PB11 layouts.
...
#define ETH_PHY_HAS_DEDICATED_RESET 1 /* 0 if PHY RESET_N hard-wired to MCU NRST */
#endif
...
#warning "BOARD_ENABLE_NAND=1: PD0/PD1 used by FSMC (NAND D2/D3) conflict with X/Y_ENA. Remap required."
#endif

/* ALM (alarm/fault) pins */
...
/* Emergency Stop (active low with pull-up, EXTI4) */
...
/* Probe input (active low with pull-up) */

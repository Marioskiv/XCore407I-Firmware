 ...
 *
 * NAND Flash (K9F1G08U0C) via FSMC 8-bit (schematic cross-check):
 *  D0  PD14 (FSMC_D0)
 *  D1  PD15 (FSMC_D1)
 *  D2  PD0  (FSMC_D2)  -> conflicts with X_ENA default
 *  D3  PD1  (FSMC_D3)  -> conflicts with Y_ENA default
 *  D4  PE7  (FSMC_D4)
 *  D5  PE8  (FSMC_D5)
 *  D6  PE9  (FSMC_D6)
 *  D7  PE10 (FSMC_D7)
 *  NOE PD4  (FSMC_NOE)
 *  NWE PD5  (FSMC_NWE)
 *  CE# PG9  (FSMC_NE2)  (previous comment listed PD7; corrected)
 *  ALE PD11 (FSMC_A16)
 *  CLE PD12 (FSMC_A17)
 *  R/B PD6  (FSMC_NWAIT)
 *  WP  (not routed / TBD on current schematic)
 * Notes:
 *  - Enabling NAND requires remapping motion ENA off PD0/PD1 (use MOTION_PROFILE_FSMC_SAFE or custom overrides).
 *  - CE# on PG9 leaves PD7 (NE1) unused for this design.
 *  - R/B# (PD6) can be optionally polled for ready/busy status.
 *
...
#endif
#ifndef BOARD_ENABLE_SDIO
#define BOARD_ENABLE_SDIO 0   /* SDIO interface (uses PC8..PC12, PD2) */
#endif
...
#endif
#ifndef MOTION_PROFILE_SDIO_SAFE
#define MOTION_PROFILE_SDIO_SAFE 0
#endif

#if MOTION_PROFILE_FSMC_SAFE && MOTION_PROFILE_SDIO_SAFE
#error "Cannot enable both MOTION_PROFILE_FSMC_SAFE and MOTION_PROFILE_SDIO_SAFE"
#endif
...
#endif
#elif MOTION_PROFILE_SDIO_SAFE
/* SDIO-safe profile: preserve PC8..PC12 and PD2 for SDIO. Use default PD0..PD3 enables (acceptable ONLY if NAND disabled).
 * If NAND also enabled raise warning: user must manually override.
 */
 ...
 */
#if BOARD_ENABLE_NAND && MOTION_PROFILE_SDIO_SAFE
#error "Invalid config: BOARD_ENABLE_NAND with MOTION_PROFILE_SDIO_SAFE (PD0/PD1 conflict). Use MOTION_PROFILE_FSMC_SAFE or override ENA pins."
#endif
#if BOARD_ENABLE_NAND && !MOTION_PROFILE_FSMC_SAFE && !MOTION_PROFILE_SDIO_SAFE
#error "Invalid config: NAND enabled but motion ENA use PD0/PD1. Enable MOTION_PROFILE_FSMC_SAFE or remap X/Y_ENA."
#endif

#if BOARD_ENABLE_SDIO && MOTION_PROFILE_FSMC_SAFE
#error "Invalid config: SDIO with MOTION_PROFILE_FSMC_SAFE (PC8..PC12 conflict). Choose SDIO_SAFE profile or custom remap."
#endif

#if BOARD_ENABLE_NAND && BOARD_ENABLE_SDIO && (MOTION_PROFILE_SDIO_SAFE || MOTION_PROFILE_FSMC_SAFE)
/* Both memories plus a motion profile still may leave residual conflicts (ALM/ENA). Provide explicit error for clarity. */
#error "Invalid config: Simultaneous NAND+SDIO with current motion profile set. Manual bespoke remap required."
#endif

/* -------------------------------------------------------------------------- */
/* NAND (K9F1G08U0C) pin macros (only if BOARD_ENABLE_NAND)                   */
/* -------------------------------------------------------------------------- */
...
#endif
/* Control signals */
#ifndef NAND_NOE_PORT
#define NAND_NOE_PORT GPIOD
#endif
#ifndef NAND_NOE_PIN
#define NAND_NOE_PIN  GPIO_PIN_4
#endif
#ifndef NAND_NWE_PORT
#define NAND_NWE_PORT GPIOD
#endif
#ifndef NAND_NWE_PIN
#define NAND_NWE_PIN  GPIO_PIN_5
#endif
#ifndef NAND_CE_PORT
#define NAND_CE_PORT GPIOG  /* CE# (NE2) */
#endif
#ifndef NAND_CE_PIN
#define NAND_CE_PIN  GPIO_PIN_9
#endif
#ifndef NAND_ALE_PORT
#define NAND_ALE_PORT GPIOD
#endif
#ifndef NAND_ALE_PIN
#define NAND_ALE_PIN  GPIO_PIN_11
#endif
#ifndef NAND_CLE_PORT
#define NAND_CLE_PORT GPIOD
#endif
#ifndef NAND_CLE_PIN
#define NAND_CLE_PIN  GPIO_PIN_12
#endif
#ifndef NAND_RB_PORT
#define NAND_RB_PORT GPIOD  /* R/B# (NWAIT) */
#endif
#ifndef NAND_RB_PIN
#define NAND_RB_PIN  GPIO_PIN_6
#endif
/* Optional WP (write protect) placeholder: define NAND_WP_PORT/PIN externally if routed. */

/* Additional compile-time sanity: if motion profile unsafe with NAND, existing errors handle PD0/PD1 conflict. */
#endif /* BOARD_ENABLE_NAND */
...
#endif
#elif MOTION_PROFILE_SDIO_SAFE
/* Keep original ALM mapping (PE0..PE3) to avoid PC10..PC12 so SDIO lines free */
...
#elif MOTION_PROFILE_SDIO_SAFE
/* Keep on PB2 BOOT1? Better leave original to avoid BOOT strapping issues unless user chooses override */
...
#endif
#elif MOTION_PROFILE_SDIO_SAFE
/* Leave probe internal so SDIO lines untouched */

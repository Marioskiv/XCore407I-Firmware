...
HEADER_FMT = '<I I H H'  # magic, seq, payloadLen, version

# Base command layout (v4): jointFreq[4]int32, setPoint[4]float, jointEnable(uint32), outputs(uint32)
BASE_CMD_FMT = '<' + 'i'*4 + 'f'*4 + 'I' + 'I'
...
class RemoraClient:
    ...
    def recv_feedback(self):
        ...
        processVar = take('<' + 'f'*4)
        inputs = take('<H')[0]; padding_take = take('<H')[0]
        faultMask = take('<I')[0]
        estop = take('<I')[0]
        jogSpeeds = take('<' + 'I'*4)
        jogTargets = take('<' + 'I'*4)
        jogDirs = take('<' + 'I'*4)
        probe = take('<I')[0]
        firmwareVersion = take('<I')[0]
        buildHash = take('<I')[0]
        heartbeat = take('<I')[0]
        uptime = take('<I')[0]
        statusFlags = take('<I')[0]
        seqGapEvents = take('<I')[0]
        crc32_val = take('<I')[0]
